...
//##########################################################################################################################################
//########################################## end block 0 start block 1 Wang ################################################################
//##########################################################################################################################################
...
uint32* find_block1_Wang(uint32 block[16], uint32 IHV[4])
{
    ...
    while (true) //meh
	{
		...
		while (counter < (1 << 12))
        {
            ...
            uint32 q_18 = f_t( Q[offset + 17], Q[offset + 16],Q[offset + 15], 17) + t_18;
            q_18 = RL(q_18, RC(17));
            q_18 += q_17;

            if (0x00020000 != ((q_18 ^ q_17) & 0xa0020000)) continue;
        
            uint32 q_19 = f_t( Q[offset + 18], Q[offset + 17],Q[offset + 16], 18) + t_19;
            q_19 = RL(q_19, RC(18));
            q_19 += q_18;
            ...
            m_0 = RR(m_0, RC(0)) - t_0;
            uint32 q_20 = f_t(q_19, q_18,q_17,19) + Q[offset + 16] + AC(19) + m_0;
            q_20 = RL(q_20,RC(19));
            q_20 += q_19;
		...
		while (counter < (1<<6))
		{
			...
			while (counter2 < (1<<5))
			{
				...
				uint32 m_10 = RR(Q[offset + 11] - q_10,17);
